...
- 阶段 0 建立的假执行拦截仍覆盖网页检索、音乐播放和外部软件控制；模型已被明确禁止把无工具结果的动作说成已完成。
...
- 仅支持新建 UTF-8 `.md`，允许位置为 Windows 桌面和 `E:\时序-output`。文件名禁止路径、非法字符和 Windows 保留名；同名文件自动增加 ` (2)`，绝不覆盖。
- 写入后重新校验文件存在性、大小和 SHA-256。成功发送 `artifact.created`，前端显示文件名、位置、大小与校验摘要；失败发送 `artifact.failed` 并保留草稿。
- 暂不支持任意目录、覆盖、删除、移动、打开文件/目录、联网检索、音乐播放或外部软件控制。
...
- 真实验收文件：`E:\时序-output\阶段2联网研究最终验收_2026-08-10.md`，读取 3 个来源，其中 2 个被识别为官方/一手来源。
